...
# --------------------------------------------------------------------------
# MSME creation request (new MSME registration via the API)
# --------------------------------------------------------------------------

class CreateMsmeRequest(BaseModel):
    """All fields required to register a brand-new MSME.
    Combines MsmeMaster + PublicSignals + ConsentFinancialSignals in one body.
    Sensible defaults are provided for optional numeric signals so the form
    can be submitted with only the essential fields filled in.
    """
    ...
    city: str
    state: str
    business_age_years: float = Field(ge=0)
    legal_structure: str
    udyam_registered: bool = False
    employee_count: int = Field(ge=0, default=1)
    credit_history_available: bool = False
    primary_sales_channel: str = "In-store"
    archetype: Archetype = Archetype.STANDARD

    # --- PublicSignals fields ---
    google_rating: float = Field(ge=0, le=5, default=3.5)
    google_review_count: int = Field(ge=0, default=0)
    positive_review_percentage: float = Field(ge=0, le=100, default=70.0)
    review_sentiment_score: float = Field(ge=-1, le=1, default=0.0)
    social_media_followers: int = Field(ge=0, default=0)
    social_engagement_rate: float = Field(ge=0, default=0.0)
    website_present: bool = False
    website_quality_score: float = Field(ge=0, le=100, default=0.0)
    website_domain_age_years: float = Field(ge=0, default=0.0)
    business_listing_consistency: float = Field(ge=0, le=100, default=50.0)
    digital_activity_score: float = Field(ge=0, le=100, default=30.0)

    # --- ConsentFinancialSignals fields ---
    average_monthly_gst_turnover: float = Field(ge=0, default=0.0)
    annual_gst_turnover: float = Field(ge=0, default=0.0)
    gst_turnover_growth_percentage: float = Field(default=0.0)
    gst_filing_timeliness_percentage: float = Field(ge=0, le=100, default=80.0)
    gst_sales_variance: float = Field(ge=0, default=20.0)
    average_monthly_upi_inflow: float = Field(ge=0, default=0.0)
    upi_transaction_count: int = Field(ge=0, default=0)
    upi_inflow_growth_percentage: float = Field(default=0.0)
    upi_refund_percentage: float = Field(ge=0, le=100, default=0.0)
    average_monthly_bank_credit: float = Field(ge=0, default=0.0)
    average_monthly_bank_debit: float = Field(ge=0, default=0.0)
    average_monthly_balance: float = Field(ge=0, default=0.0)
    monthly_cash_surplus: float = Field(default=0.0)
    existing_monthly_loan_obligation: float = Field(ge=0, default=0.0)
    cheque_bounce_count: int = Field(ge=0, default=0)
    payment_failure_percentage: float = Field(ge=0, le=100, default=0.0)
    epfo_employee_count: int = Field(ge=0, default=0)
    epfo_employee_growth_percentage: float = Field(default=0.0)
    epfo_contribution_timeliness_percentage: float = Field(ge=0, le=100, default=80.0)
    cash_flow_volatility: float = Field(ge=0, default=20.0)
