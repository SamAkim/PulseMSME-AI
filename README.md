...
- **Add New MSME** — credit officers can register a brand-new MSME via a two-step wizard form directly from the UI; the new business is immediately available for the full assessment pipeline.
...
```
Register / Select MSME → Analyse public signals → Preliminary assessment
...
Credit officers can register a new business directly from the **MSME Selection** screen without touching any files or restarting the server.

**How it works:**
1. Click **"+ Add New MSME"** on the selection page.
2. **Step 1 — Business Profile**: fill in business name, sector, city, state, age, legal structure, employee count, Udyam registration status, and sales channel.
3. **Step 2 — Financial Signals**: enter public signals (Google rating, social media, website) and consent financial signals (GST turnover, UPI inflow, banking data, EPFO figures).
4. Submit → the MSME is instantly available in the list with a unique `CUSTOM_XXX` ID and can go through the complete pipeline (public score → consent → enhanced score → report).

> New MSMEs are held **in memory** for the lifetime of the server process (no file writes by design). They reset on server restart.

**API endpoint added:** `POST /api/msme` — accepts a `CreateMsmeRequest` body and returns the new `MsmeListItem`.
...
|---|---|---|
| `GET` | `/api/msme` | List all MSMEs (supports `search`, `sector`, `city`, `riskBand` query params) |
| `POST` | `/api/msme` | **Register a new MSME** (returns new `MsmeListItem` with auto-generated ID) |
| `GET` | `/api/msme/{id}` | Get MSME profile |
| `GET` | `/api/msme/{id}/public-assessment` | Run Layer 1 public score |
| `POST` | `/api/msme/{id}/consent` | Grant consent for data sources |
| `POST` | `/api/msme/{id}/assess` | Run full LangGraph pipeline (sync) |
| `GET` | `/api/msme/{id}/assess/stream` | Stream pipeline status over SSE |
| `POST` | `/api/msme/{id}/chat` | Chat with the AI credit assistant |
| `GET` | `/api/msme/{id}/report` | Get printable report payload |
...
When running the backend, watch the terminal:
- **No warnings** → LLM is working.
- `WARNING pulsemsme.llm: LLM provider gemini failed: ...` → LLM unavailable, falling back to templates.

In the UI, rich context-specific narrative text = LLM active. Short generic text = fallback mode.
...
- All pre-seeded data is synthetic and generated with a fixed seed — it is not real MSME data.
- New MSMEs added via the UI form are held in memory only and reset on server restart.
...
- Add a persistence layer (assessment history, audit trail, consent ledger, persisted custom MSMEs).
